---
layout: book-review
title: Monk & Robot - A Psalm for the Wild-Built
author: Becky Chambers
olid: OL32177625M # use Open Library ID to fetch cover (if no `cover` is provided)
isbn: 9781250236227 # use ISBN to fetch cover (if no `olid` is provided, dashes are optional)
categories: science-fiction hopepunk slice-of-life heartwarming
tags: top-10
buy_link: https://www.amazon.co.uk/Monk-Robot-Wild-Built-Prayer-Crown-Shy/dp/1250386330/
date: 2021-07-13
started: 2021-07-13
finished: 2021-08-09
released: 2021
stars: 5
status: Finished
---

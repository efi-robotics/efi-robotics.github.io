---
layout: book-review
title: Monk & Robot - A Prayer for the Crown-Shy
author: Becky Chambers
olid: OL35034781M # use Open Library ID to fetch cover (if no `cover` is provided)
isbn: 9781250236234 # use ISBN to fetch cover (if no `olid` is provided, dashes are optional)
categories: science-fiction hopepunk slice-of-life heartwarming
tags: top-10
buy_link: https://www.amazon.co.uk/Monk-Robot-Wild-Built-Prayer-Crown-Shy/dp/1250386330/
date: 2022-07-13
started: 2022-07-13
finished: 2022-07-18
released: 2022
stars: 5
status: Finished
---
